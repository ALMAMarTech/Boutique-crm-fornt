...
export class AuthService {
  ...
  get usuario(): Usuario {
    if (localStorage.getItem('user_boutique') != null) {
      let localValue = localStorage.getItem('user_boutique');
      if (localValue == null) {
        return new Usuario();
      } else {
        return JSON.parse(localValue) as Usuario;
      }
    }
    return new Usuario();
  }
  ...
  guardarUsuario(accessToken: string) {
    ...
    localStorage.setItem('user_boutique', JSON.stringify(this._usuario));
  }

  get token(): string {
    if (this._token != '') {
      return this._token;
    } else if (this._token == '' && localStorage.getItem('token_boutique') != null) {
      let localValue = localStorage.getItem('token_boutique');
      if (localValue == null) {
        return '';
      } else {
        return localValue;
  ...
  get tokenRefresh(): string {
    ...
    } else if (this._tokenRefresh == '' && localStorage.getItem('tokenRefresh_boutique') != null) {
      let localValue = localStorage.getItem('tokenRefresh_boutique');
      if (localValue == null) {
        return '';
      } else {
        return localValue;
  ...
  guardarToken(accessToken: string) {
    ...
    localStorage.setItem('token_boutique', this._token);
  }

  guardarRefreshToken(refreshToken: any) {
    this._tokenRefresh = refreshToken;
    localStorage.setItem('tokenRefresh_boutique', this._tokenRefresh);
  }
